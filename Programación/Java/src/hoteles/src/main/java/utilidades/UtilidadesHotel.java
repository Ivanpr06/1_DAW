...
public class UtilidadesHotel {
    ...
    public static List<Hotel> getConAlMenos3Servicios(List<Hotel> hoteles, List<ServicioHotel> serviciosRequeridos){
        ...
        for(Hotel hotel : hoteles){
            int contador = 0;
            for(ServicioHotel servicio : serviciosRequeridos){
    ...
    public static Map<TipoHabitacion,Integer> getNumReservasPorTipoHabitacion(List<Habitacion> habitaciones){
        ...
        for(Habitacion habitacion : habitaciones){
            if(mapa.containsKey(habitacion.getTipoHabitacion())){
                mapa.put(habitacion.getTipoHabitacion(), mapa.get(habitacion.getTipoHabitacion()) + habitacion.getReservas().size());
            }else{
                mapa.put(habitacion.getTipoHabitacion(), habitacion.getReservas().size());
            }
        }
        return mapa;
    }
...
//            if(mapaFinal.containsKey(viajero.getFechaNacimiento())){
//                if(mapaFinal.get(Period.between(viajero.getFechaNacimiento(), LocalDate.now()).getYears()) <= 5){
//
//                } else if (mapaFinal.get(Period.between(viajero.getFechaNacimiento(), LocalDate.now()).getYears()) <= 12) {
//
//                }
//            }
//        }
    ...
    public static boolean habitacionDisponibleFechas(LocalDate fechaInicio , LocalDate fechaFin, Habitacion habitacion){
        for(Reserva reserva : habitacion.getReservas()){
            if(reserva.getFechaInicio().equals(fechaInicio) && reserva.getFechaInicio().isAfter(fechaFin)){
                return false;
            }else{
                return true;
            }
